...
class _ScheduledClassesPageState extends State<ScheduledClassesPage> {
  ...
  Future<void> _loadUserId() async {
    ...
    if (userId == null) {
      ...
      scheduledClassesFuture = Future.value([]); // Lista vacía si no se carga el userId
    } else {
  ...
  Future<void> _deleteScheduledClass(int scheduleId) async {
    ...
    _loadUserId(); // Recarga el userId para actualizar las reservas
  }

  Future<void> _refreshScheduledClasses() async {
    // Llama a la función para obtener las clases programadas
    setState(() {
      scheduledClassesFuture = _fetchScheduledClasses();
    });
  }
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return RefreshIndicator(
                  onRefresh: _refreshScheduledClasses, // Función que se llama al refrescar
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
  title: Text(
    "Fecha: ${DateFormat('dd/MM/yyyy').format(scheduledClass.reservationDate.toLocal())} - Hora: ${scheduledClass.reservationTime}",
  ),
